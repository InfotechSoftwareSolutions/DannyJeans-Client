...
useEffect(() => {
  ...
  const handleDeleteCategory = async (categoryId) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/category/admin/${categoryId}`, { method: "DELETE" });
  ...
  console.log(editingCategory,"==editingCategory out"); 
  ...
  const handleSaveCategory = async () => {
    ...
    console.log(categoryData,"==categoryDatapppp");
...
clearState();
  ...
  const handleAddCategory = async () => {
    ...
    try {
      ...
      setCategories([...categories, { ...savedCategory, id: categories.length + 1 }]);
      setNewCategory({ name: "", description: "", parentCategory: "", published: false, image: null });
  ...
  const handleToggle = async(productId)=> {
   ...
   console.log("productId",productId);
    ...
    console.log(response,"handleToggle");
    ...
    <div className="p-4 bg-gray-100 min-h-screen">
      ...
      <div className="flex flex-wrap justify-between gap-2 mb-4">
        <div className="flex flex-wrap gap-2">
          <button className="px-4 py-2 bg-white border rounded shadow">Export</button>
          <button className="px-4 py-2 bg-white border rounded shadow">Import</button>
        </div>
        <button
...
<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          <button className="px-4 py-2 bg-green-600 text-white rounded w-full sm:w-auto">Filter</button>
          <button className="px-4 py-2 bg-gray-300 rounded w-full sm:w-auto">Reset</button>
        </div>
      </div>
          ...
          <tbody>
            {categories.map((category) => (
              <tr key={category.id} className="border-b">
                ...
                <td className="p-2 text-center">
                <button onClick={() => handleEditCategory(category)} className="text-blue-500">✏️</button>
                </td>
                <td className="p-2 text-center">
...
<div className="flex flex-wrap justify-between items-center mt-4">
        <button className="px-4 py-2 bg-gray-300 rounded w-full sm:w-auto">Previous</button>
        <span>Page 1 of 1</span>
        <button className="px-4 py-2 bg-gray-300 rounded w-full sm:w-auto">Next</button>
      </div>
        ...
        <div className="fixed inset-0 flex justify-end bg-gray-900 bg-opacity-50">
          <div className="bg-white p-6 shadow-lg w-1/3 h-full overflow-auto">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={clearState} className="text-lg">✖</button>
            </div>
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setIsModalOpen(false)} className="px-4 py-2 bg-red-500 text-white rounded">Cancel</button>
              <button onClick={handleSaveCategory} className="px-4 py-2 bg-green-500 text-white rounded">
